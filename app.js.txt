/*
TRILLIONS DUAL UI AUTO ORCHESTRATION MASTER
===================================
But:
...
const STATE = {
  ...
  autoOrchestrator: {
    enabled: false,
    profile: "EXPERT",
    mode: "AUTO_CALCUL_VISIBLE_ONLY",
    autoMining: false,
    autoMiningRequiresExplicitStart: true,
    lastDecision: null,
    decisions: [],
    rules: {
      noHiddenMining: true,
      noFakeMetrics: true,
      realSourceRequired: true,
      maxCpuLoadEco: 0.70,
      maxCpuLoadExpert: 1.40,
      maxCpuLoadExtreme: 2.40
    }
  },
  metrics: {}
};
...
function autoProfileLimits(profile) {
  const p = String(profile || "EXPERT").toUpperCase();
  if (p === "ECO") return { profile: "ECO", maxLoad: 0.70, modules: ["repo", "btc-readonly", "ai-standby"] };
  if (p === "EXTREME") return { profile: "EXTREME", maxLoad: 2.40, modules: ["repo", "btc", "web3-readonly", "mining-bridge-visible", "ai"] };
  return { profile: "EXPERT", maxLoad: 1.40, modules: ["repo", "btc", "web3-readonly", "ai"] };
}

function pushAutoDecision(decision) {
  STATE.autoOrchestrator.lastDecision = decision;
  STATE.autoOrchestrator.decisions.push(decision);
  if (STATE.autoOrchestrator.decisions.length > 120) STATE.autoOrchestrator.decisions.shift();
}

function autoDecision(reason = "tick") {
  const limits = autoProfileLimits(STATE.autoOrchestrator.profile);
  ...
  const decision = {
    ts: now(),
    reason,
    enabled: !!STATE.autoOrchestrator.enabled,
    profile: limits.profile,
    load1: Number(load.toFixed(3)),
    heapMB: Math.round(mem.heapUsed / 1024 / 1024),
    actions: [],
    blocked: []
  };

  if (!STATE.autoOrchestrator.enabled) {
    decision.actions.push("AUTO_ORCHESTRATION_STANDBY");
  } else {
    if (load > limits.maxLoad) {
      decision.actions.push("THROTTLE_RUNTIME");
      decision.blocked.push("HEAVY_WORKLOAD_BLOCKED_BY_LOAD");
    } else {
      decision.actions.push("SCAN_RUNTIME_ALLOWED");
      decision.actions.push("BTC_READONLY_REFRESH_ALLOWED");
      decision.actions.push("STATE_SYNC_ALLOWED");
    }

    if (STATE.autoOrchestrator.autoMining === true) {
      decision.blocked.push("AUTO_MINING_BLOCKED_NO_HIDDEN_MINING");
      decision.actions.push("VISIBLE_MINING_BRIDGE_ONLY");
    } else {
      decision.actions.push("AUTO_CALCUL_ONLY");
    }

    if (STATE.miningBridge.connected) decision.actions.push("REAL_MINING_METRICS_BRIDGE_MONITOR");
    if (STATE.web3.status === "ACTIVE") decision.actions.push("WEB3_READONLY_MONITOR");
    if (STATE.btcTerminal.status === "ACTIVE") decision.actions.push("BTC_TERMINAL_MONITOR");
  }

  pushAutoDecision(decision);
  neuralPulse("auto", "Auto orchestration decision", decision);
  return decision;
}

function setAutoEnabled(enabled) {
  STATE.autoOrchestrator.enabled = !!enabled;
  log("AUTO", STATE.autoOrchestrator.enabled ? "AUTO ORCHESTRATION ON" : "AUTO ORCHESTRATION OFF");
  return autoDecision("manual-toggle");
}

function setAutoProfile(profile) {
  const p = String(profile || "EXPERT").toUpperCase();
  STATE.autoOrchestrator.profile = ["ECO", "EXPERT", "EXTREME"].includes(p) ? p : "EXPERT";
  log("AUTO", "PROFILE -> " + STATE.autoOrchestrator.profile);
  return autoDecision("profile-change");
}
...
function snapshot() {
  ...
  return {
    ...
    safety: STATE.safety || {},
    autoOrchestrator: STATE.autoOrchestrator || {},
...
<div id="navtabs">
  ...
  <div class="ntab" onclick="showPanel('auto')">AUTO</div>
...
<div id="panel-auto" class="panel">
  <div class="cockpit">
    <div class="card wide2">
      <h3>AUTO ORCHESTRATION</h3>
      <div class="kv" id="auto-kv"></div>
      <br>
      <button onclick="autoEnable(true)">AUTO ON</button>
      <button onclick="autoEnable(false)">AUTO OFF</button>
      <button onclick="autoProfile('ECO')">ECO</button>
      <button onclick="autoProfile('EXPERT')">EXPERT</button>
      <button onclick="autoProfile('EXTREME')">EXTREME</button>
      <button onclick="autoDecision()">DECISION</button>
    </div>
    <div class="card wide2">
      <h3>DERNIÈRE DÉCISION</h3>
      <pre id="auto-decision" style="white-space:pre-wrap;font-size:10px;max-height:220px;overflow:auto"></pre>
    </div>
    <div class="card wide4">
      <h3>RÈGLE</h3>
      <div style="font-size:12px;line-height:2">
        AUTO-ORCHESTRATION = calcul visible, monitoring, adaptation profils, décision runtime.<br>
        AUTO-MINING CACHÉ = bloqué.<br>
        Matériel émulé, métriques réelles seulement si source connectée.
      </div>
    </div>
  </div>
</div>
...
function render(s){
  ...
  // Auto orchestration
  if (s.autoOrchestrator) {
    const ao = s.autoOrchestrator;
    const autoKv = document.getElementById("auto-kv");
    const autoDec = document.getElementById("auto-decision");
    if (autoKv) autoKv.innerHTML = kvHtml({
      Enabled: ao.enabled ? "ON" : "OFF",
      Profile: ao.profile || "EXPERT",
      Mode: ao.mode || "AUTO_CALCUL_VISIBLE_ONLY",
      AutoMining: ao.autoMining ? "BLOCKED_VISIBLE_ONLY" : "OFF",
      HiddenMining: "BLOCKED"
    });
    if (autoDec) autoDec.textContent = JSON.stringify(ao.lastDecision || {}, null, 2);
  }
...
async function autoEnable(enabled){
  await fetch("/api/auto/enable",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({enabled})});
  load();
}
async function autoProfile(profile){
  await fetch("/api/auto/profile",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({profile})});
  load();
}
async function autoDecision(){
  await fetch("/api/auto/decision",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({reason:"ui"})});
  load();
}
...
APP.get("/api/auto/state", (req,res) => res.json({ ok:true, autoOrchestrator: STATE.autoOrchestrator, decision: autoDecision("api-state") }));
APP.post("/api/auto/enable", (req,res) => {
  const decision = setAutoEnabled(!!req.body?.enabled);
  IO.emit("state", snapshot());
  res.json({ ok:true, autoOrchestrator: STATE.autoOrchestrator, decision });
});
APP.post("/api/auto/profile", (req,res) => {
  const decision = setAutoProfile(req.body?.profile || "EXPERT");
  IO.emit("state", snapshot());
  res.json({ ok:true, autoOrchestrator: STATE.autoOrchestrator, decision });
});
APP.post("/api/auto/decision", (req,res) => {
  const decision = autoDecision(String(req.body?.reason || "manual"));
  IO.emit("state", snapshot());
...
async function mainLoop() {
  STATE.tick++;
  if (STATE.autoOrchestrator && STATE.autoOrchestrator.enabled) autoDecision("main-loop");
...
async function boot() {
  ...
  SERVER.listen(PORT, "0.0.0.0", () => {
    ...
    log("AUTO", "Auto orchestration ready: ECO / EXPERT / EXTREME");
  });
}
