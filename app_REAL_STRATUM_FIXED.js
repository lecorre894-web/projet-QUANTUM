...
const tabs = ["VUE_REELLE","ALGORITHMES","STRATUM_POOLS","STRATUM_NATIVE","WEBRTC_MESH","WALLET_VIEWER","MATERIEL","EXPERT_BBB","RESEAU","JOURNAUX","PARAMETRES","IA_SOUVERAINE","HONESTY_GUARD","VERS_EXE"];
...
function renderTab(t){
  ...
  if(t==="STRATUM_POOLS"){
    ...
    html += card("Pools / rigs", '<pre>'+esc(JSON.stringify(s.mining,null,2))+'</pre>');
  }
  ...
  if(t==="STRATUM_NATIVE"){
    ...
    html += card("Jobs Stratum",
      '<pre>'+esc(JSON.stringify(s.stratumNative?.lastJobs || [], null, 2))+'</pre>'
    );

    html += card("Logs Stratum",
      '<pre>'+esc(JSON.stringify(s.stratumNative?.lastMessages || [], null, 2))+'</pre>'
    );
  }
  ...
  if(t==="WEBRTC_MESH"){
    ...
    html += card("MESH EVENTS",
      '<pre>'+esc(JSON.stringify(s.webrtc||{},null,2))+'</pre>'
    );
  }
  ...
  if(t==="WALLET_VIEWER"){
    ...
    html += card("Wallet Runtime",
      '<pre>'+esc(JSON.stringify(s.walletViewer||{},null,2))+'</pre>'
    );
  }
