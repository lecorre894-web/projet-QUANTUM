...
function renderHeader(){
  ...
  el("hHash").textContent = formatHashrate(s.mining?.totals?.hashrate||0);
...
body{margin:0;background:radial-gradient(circle at top,#052016,#000 45%);color:var(--txt);font-family:Consolas,Menlo,monospace;overflow:auto;touch-action:auto}
#matrix{position:fixed;inset:0;z-index:-1;opacity:.16}
.app{min-height:100vh;height:auto;overflow:auto;padding-bottom:140px;display:grid;grid-template-rows:auto auto 1fr auto;gap:8px;padding:10px}
...
.content{min-height:0;overflow:visible;display:grid;grid-template-columns:repeat(auto-fit,minmax(330px,1fr));gap:10px}.card{border:1px solid var(--line);background:rgba(3,17,10,.88);padding:10px;min-height:140px}.card h3{margin:0 0 8px;color:var(--cyan);font-size:13px;letter-spacing:.1em}
...
// DEMO LIVE FEED
setInterval(() => {
  if(state.runtime.power || state.runtime.minerX){
    const base = state.runtime.activeAlgo.includes("SHA") ? 125000000 :
                 state.runtime.activeAlgo.includes("RANDOMX") ? 2400 :
                 1.24;

    const liveHash = base + (Math.random() * base * 0.08);

    state.mining.connected = true;
    state.mining.rigs = [{
      id:"genesis-rig",
      coin:"BBB",
      algo:state.runtime.activeAlgo,
      hashrate:liveHash,
      unit: state.runtime.activeAlgo.includes("SHA") ? "H/s" :
            state.runtime.activeAlgo.includes("RANDOMX") ? "H/s" : "GH/s",
      accepted: Math.floor(Math.random()*4000),
      rejected: Math.floor(Math.random()*6),
      powerW: 420 + Math.floor(Math.random()*120),
      tempC: 52 + Math.floor(Math.random()*16)
    }];

    state.mining.totals = miningTotals(state.mining.rigs);

    pushLimited(state.mining.history,{
      ts:now(),
      totals:state.mining.totals,
      rigs:1
    },LIMITS.history);
    ...
    state.mining.accumulated.amount +=
      (liveHash / 1000000000) * 0.000001;

    state.mining.accumulated.usdValue =
      state.mining.accumulated.amount * 0.42;
  }
}, 2500);
