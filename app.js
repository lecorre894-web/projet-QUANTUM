/*
QUANTUM / BBB GENESIS TRILLIONS MASTER — EMU HW / REAL DATA — RESTRUCTURED SAFE RUNTIME
Version: V5_EMU_HARDWARE_REAL_DATA
Objectif:
...
const APP_NAME = "BBB GENESIS TRILLIONS MASTER — EMU HW / REAL DATA";
const VERSION = "V5_EMU_HARDWARE_REAL_DATA";
...
const state = {
  ...
  emulationPolicy: {
    ...
    generatedNumbers: false,
    fakeHashrate: false,
    fakeBalances: false,
    fakeShares: false,
    rule: "NEVER_INVENT_METRICS"
  }
};
...
function guardStatus() {
  return {
    ...
    privateKeys: "BLOCKED",
    emulatedHardware: "YES",
    realDataOnly: "YES_WHEN_CONNECTED",
    generatedNumbers: "BLOCKED"
  };
}
...
function snapshot() {
  return {
    ...
    notes: state.notes,
    emulationPolicy: state.emulationPolicy
  };
}
...
app.get("/api/xmrig/command", (req, res) => {
  const cmd = `xmrig --http-host=127.0.0.1 --http-port=18080 --algo=rx/0 --url=${process.env.POOL || "POOL_REQUIRED_FOR_REAL_SOURCE:3333"} --user=${process.env.WALLET || "WALLET_REQUIRED_FOR_REAL_POOL"} --worker=${process.env.WORKER || "bbb-genesis-worker"}`;
...
const tabs = ["VUE_REELLE","EMULATION_POLICY","ALGORITHMES","STRATUM_POOLS","STRATUM_NATIVE","BLOCKCHAIN_RUNTIME","WEBRTC_MESH","WALLET_VIEWER","MATERIEL","EXPERT_BBB","RESEAU","JOURNAUX","PARAMETRES","IA_SOUVERAINE","HONESTY_GUARD","VERS_EXE"];
...
function renderTab(t){
  ...
  if(t==="EMULATION_POLICY"){
    html += card("POLITIQUE ÉMULATION",
      kv("MATÉRIEL", "ÉMULÉ") +
      kv("DONNÉES", "RÉELLES SI SOURCE CONNECTÉE") +
      kv("CHIFFRES INVENTÉS", "BLOQUÉS") +
      kv("HASHRATE", "SOURCE REQUISE") +
      kv("WALLET", "READONLY / PAS DE CLÉ PRIVÉE") +
      kv("RÈGLE", "NEVER_INVENT_METRICS")
    );
    html += card("État policy", '<pre>'+esc(safeJson(s.emulationPolicy || {},4000))+'</pre>');
  }

  if(t==="ALGORITHMES"){
    html += card("Stack algorithmes", kv("BBB PoW V2","emulated hardware layer / no generated metrics") + kv("SHA256","WebCrypto legacy") + kv("RandomX","External XMRig bridge visible") + kv("XMRig API",s.mining?.xmrigApi || ""));
    html += card("Commande XMRig visible", '<pre>'+esc("xmrig --http-host=127.0.0.1 --http-port=18080 --algo=rx/0 --url=POOL_REQUIRED_FOR_REAL_SOURCE:3333 --user=WALLET_REQUIRED_FOR_REAL_POOL --worker=bbb-genesis-worker")+'</pre>');
  }
  ...
  if(t==="STRATUM_NATIVE"){
    ...
    html += card("Contrôle Stratum", '<button class="primary" onclick="stratumConnectUI()">CONNECT NATIVE</button><button class="danger" onclick="stratumDisconnectUI()">DISCONNECT</button><button onclick="stratumPingUI()">PING</button><div class="small">Client TCP/TLS visible. Matériel émulé. Données réelles seulement si source connectée. Aucun hash caché, aucune clé privée.</div>');
  ...
  if(t==="VERS_EXE"){
    html += card("Build", kv("Codespaces","node app.js") + kv("Dependencies","npm install") + kv("Download","app.js.txt -> rename app.js") + kv("Mode","emulated hardware / real data only runtime"));
  }
...
<div class="app">
  <div class="header">
    <div><div class="title">BBB GENESIS TRILLIONS MASTER — EMU HW / REAL DATA</div><div class="sub">SUPREME BRAIN • HARDWARE EMULATED • REAL DATA ONLY • HONESTY GUARD</div></div>
    <div class="badges">
      <div class="badge">HASHRATE <b id="hHash">WAIT SOURCE</b></div>
...
app.get("/api/emulation/policy", (req, res) => res.json({
  ok: true,
  mode: "EMULATED_HARDWARE_REAL_DATA",
  hardware: "EMULATED",
  metrics: "REAL_ONLY_WHEN_SOURCE_CONNECTED",
  generatedNumbers: false,
  fakeHashrate: false,
  fakeBalances: false,
  fakeShares: false,
