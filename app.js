/*
TRILLIONS SUPREME BRAIN V9 — GENESIS SOVEREIGN COCKPIT
======================================================
Objectif demandé :
- Chat IA "GENESIS Super IA Suprême Souveraine" via Pollinations gratuit, sans clé
- Chat terminal libre pour poser des milliards de sujets/questions
- Prometheus réel : endpoint /metrics au format Prometheus, sans dépendance payante
- Onglets petits/compacts : OK / ACTIF / STANDBY / POSSIBLE
- Voir les modules s'activer en temps réel
- Que du réel côté état : statut basé sur détection runtime réelle, pas hallucination
- Web3 en lecture seule
- Aucune seed, aucune clé privée, aucune transaction automatique, aucun mining automatique
...
const CONFIG = {
  APP_NAME: "TRILLIONS_SUPREME_BRAIN_V9_GENESIS",
  PORT: Number(process.env.PORT || 3000),
  BUILD: "GENESIS_SOVEREIGN_FREE_RUNTIME",
  AI_NAME: "GENESIS Super IA Suprême Souveraine",
...
function time(){ return new Date().toLocaleTimeString("fr-FR"); }

function emit(name, data){ IO.emit(name, data); }

function log(channel, message, level="info"){
  const item = { ts: now(), channel, message, level };
...
function fileInfo(rel){
  try{
    ...
    if(!full.startsWith(process.cwd())) return { present:false };
...
function setTab(id, label, status, details="", action=null){
  STATE.tabs[id] = {
    id, label, status, details, action,
    updated: now()
  };
}
...
function scanRuntime(){
  ...
  setTab("dashboard", "Dashboard UI", "ACTIVE", "Socket.IO + cockpit mobile");
  ...
  setTab("pollinations", "GENESIS IA", STATE.ai.status, STATE.ai.error || "Chat gratuit via Pollinations");
      ...
      messages: [
        {
          role:"system",
          content:
            "Tu es GENESIS Super IA Suprême Souveraine, intégrée au runtime TRILLIONS. " +
            "Tu réponds en français, clairement, techniquement. " +
            "Tu n'inventes pas d'état système : si une info n'est pas dans l'état fourni, tu dis que ce n'est pas confirmé. " +
            "Tu respectes les garde-fous : aucune seed, aucune clé privée, aucune transaction automatique, aucun gain crypto promis."
        },
        ...messages
      ],
      temperature: 0.35
...
APP.get("/api/health",(req,res)=>res.json({ok:true, app:CONFIG.APP_NAME, safety:STATE.safety, metrics:STATE.metrics}));
...
APP.post("/api/mining",(req,res)=>{
  ...
  STATE.miningBridge.rigs = rigs.slice(0,64).map((r,i)=>({
  ...
  log("MINING",`${STATE.miningBridge.rigs.length} rig(s) métriques reçues`);
...
IO.on("connection",(socket)=>{
  ...
  log("SOCKET","Dashboard connecté "+socket.id.slice(0,8));
  socket.emit("log",{ts:now(),channel:"GENESIS",message:"Tchat IA libre prêt — Pollinations gratuit"});
  socket.on("disconnect",()=>CLIENTS.delete(socket.id));
});

function boot(){
  scanRuntime();
  log("BOOT","TRILLIONS GENESIS V9 démarré");
  log("GENESIS","Super IA Suprême Souveraine prête via Pollinations");
  log("PROMETHEUS","Endpoint /metrics actif");
  log("SAFE","No seed / no private key / no auto transaction / factual status only");
  setInterval(()=>{
    STATE.tick++;
    scanRuntime();
    const msgs=["Runtime vivant","Onglets statut réel mis à jour","Prometheus prêt","Genesis IA standby/active selon usage","Web3 lecture seule possible","Terminal libre actif"];
    log("TICK",msgs[STATE.tick % msgs.length]);
  },5000);
}

boot();
SERVER.listen(process.env.PORT || 3000,"0.0.0.0",()=>log("HTTP","Port "+(process.env.PORT || 3000)+" actif"));
