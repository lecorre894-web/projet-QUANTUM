...
const tabs = ["VUE_REELLE","ALGORITHMES","STRATUM_POOLS","MATERIEL","EXPERT_BBB","RESEAU","JOURNAUX","PARAMETRES","IA_SOUVERAINE","HONESTY_GUARD","VERS_EXE"];
...
function renderHeader(){
  ...
  el("hHash").textContent = Number(s.mining?.totals?.hashrate||0).toFixed(2);
  el("hAlgo").textContent = s.runtime?.activeAlgo || "BBB_POW_V2";
  el("hStatus").textContent = s.runtime?.power ? "ON" : "OFF";
  el("hAi").textContent = s.ai?.ready ? "SOVEREIGN" : "OFF";
}
function renderTabs(){
  const box=el("tabs");
...
function renderTab(t){
  ...
  if(t==="VUE_REELLE"){
    html += card("Runtime central", '<div class="metrics">'+metric("HASHRATE",Number(s.mining?.totals?.hashrate||0).toFixed(2))+metric("RIGS",s.mining?.rigs?.length||0)+metric("ACCEPTED",s.mining?.totals?.accepted||0)+metric("POWER",s.runtime?.power?"ON":"OFF")+metric("MINER X",s.runtime?.minerX?"ON":"OFF")+metric("XMRIG",s.mining?.xmrigOnline?"LIVE":"OFF")+'</div>');
  ...
  if(t==="STRATUM_POOLS"){ html += card("Pools / rigs", '<pre>'+esc(JSON.stringify(s.mining,null,2))+'</pre>'); }
  ...
  if(t==="RESEAU"){ html += card("API / Network", kv("/api/state","OK")+kv("/api/mining","POST metrics")+kv("/api/chat","Pollinations")+kv("/api/web3/probe","read-only")); }
  if(t==="JOURNAUX"){ html += card("Terminal central", '<div id="terminal"></div>') + card("Events", '<pre>'+esc(JSON.stringify((s.events||[]).slice(-40),null,2))+'</pre>'); }
  if(t==="PARAMETRES"){ html += card("Configuration", '<pre>'+esc(JSON.stringify({app:s.app,version:s.version,mode:s.mode,runtime:s.runtime,modules:s.modules},null,2))+'</pre>'); }
  if(t==="IA_SOUVERAINE"){ html += card("Chat IA", '<div id="chatbox">'+esc(s.ai?.lastReply||"")+'</div><div class="row"><input id="chat" placeholder="Question au runtime..."><button onclick="sendChat(false)">SEND</button><button onclick="sendChat(true)">+STATE</button></div>'); }
  if(t==="HONESTY_GUARD"){ html += card("HONESTY GUARD", '<pre>'+esc(JSON.stringify(s.guard,null,2))+'</pre>'); }
  ...
  out.innerHTML=html; if(t==="VUE_REELLE") drawChart(); if(el("terminal")) C.logs.slice(-260).forEach(()=>{}), addLog({ts:new Date().toISOString(),channel:"VIEW",message:"terminal rendered"});
}
function drawChart(){
...
const socket=io(); socket.on("log",x=>{addLog(x); if(window.activeTab==="JOURNAUX"||window.activeTab==="VUE_REELLE")renderTab(window.activeTab||"VUE_REELLE")}); socket.on("event",addEvent); socket.on("mining",m=>{ if(C.state){C.state.mining=m; renderHeader(); drawChart();} }); socket.on("state",s=>{C.state=s;renderHeader();renderTab(window.activeTab||"VUE_REELLE")});
document.addEventListener("DOMContentLoaded",()=>{window.activeTab="VUE_REELLE"; renderTabs(); matrix(); load(); setInterval(load,15000);});
`;
}
...
body{margin:0;background:radial-gradient(circle at top,#052016,#000 45%);color:var(--txt);font-family:Consolas,Menlo,monospace;overflow:hidden}
#matrix{position:fixed;inset:0;z-index:-1;opacity:.16}
.app{height:100vh;display:grid;grid-template-rows:auto auto 1fr auto;gap:8px;padding:10px}
...
.tabs{display:flex;gap:5px;overflow-x:auto;border:1px solid #074;padding:7px;background:#020806}.tab{white-space:nowrap;border:1px solid #096;background:#05120d;color:#b8ffdc;padding:7px 10px;font-size:11px;cursor:pointer}.tab.on{background:#57efad;color:#02150c;font-weight:900}
.content{min-height:0;overflow:auto;display:grid;grid-template-columns:repeat(auto-fit,minmax(330px,1fr));gap:10px}.card{border:1px solid var(--line);background:rgba(3,17,10,.88);padding:10px;min-height:140px}.card h3{margin:0 0 8px;color:var(--cyan);font-size:13px;letter-spacing:.1em}
...
#terminal{height:42vh;overflow-y:auto;background:#01050f;padding:12px;border:2px solid #00ddaa;box-shadow:0 0 28px rgba(0,255,170,.25);font-size:12px;line-height:1.45}.ts{color:#6b9}.line{margin:4px 0}.line.user{color:var(--mag);border-left:5px solid var(--mag);padding-left:8px}.line.ai{color:#ffff66}
